            ...
            return DataTables::of($query)
                ...
                ->editColumn('event', function ($log) {
                    if ($log->event == 'created')
                        return '<span class="px-3 py-1 bg-green-100 text-green-700 rounded-full text-lg font-bold border border-green-200">TAMBAH</span>';
                    if ($log->event == 'updated')
                        return '<span class="px-3 py-1 bg-blue-100 text-blue-700 rounded-full text-lg font-bold border border-blue-200">UPDATE</span>';
                    if ($log->event == 'deleted')
                        return '<span class="px-3 py-1 bg-red-100 text-red-700 rounded-full text-lg font-bold border border-red-200">HAPUS</span>';
                    return '<span class="px-3 py-1 bg-gray-100 text-gray-700 rounded-full text-lg font-bold uppercase">' . e($log->event) . '</span>';
                })
                    ...
                    if ($log->properties && $log->properties->has('attributes')) {
                        ...
                        foreach ($log->properties['attributes'] as $key => $val) {
                            if (!in_array($key, ['updated_at', 'created_at'])) {
                                $html .= '<div class="text-xl mb-1 border-b border-slate-100 pb-1">
