...
<div id="sidebar-overlay"
    class="fixed inset-0 bg-black/60 z-40 hidden backdrop-blur-sm transition-opacity cursor-pointer md:hidden"></div>

<button id="sidebar-float-toggle" class="fixed top-6 left-0 z-[60] 
...
<aside id="sidebar-menu" class="fixed inset-y-0 left-0 z-50 w-80 min-h-screen bg-white text-gray-700 border-r border-gray-200 shadow-2xl 
    ...
    <nav class="px-4 py-2 space-y-2 text-base overflow-y-auto h-[calc(100vh-150px)]">
        ...
        <button id="berita-acara-toggle"
            ...
            <span class="glyphicon glyphicon-chevron-down text-xl transition-transform duration-200"
                id="berita-acara-icon"></span>
        </button>
        ...
        <div id="berita-acara-submenu" class="overflow-hidden transition-all duration-300 ease-in-out max-h-0">
            ...
            @foreach([2026, 2025, 2024, 2023, 2022, 2021, 2020] as $year)
                <a href="{{ route('dashboard', ['tahun' => $year]) }}"
                    class="flex items-center gap-3 px-8 py-2 rounded-lg transition-all duration-200 no-underline {{ request('tahun') == $year ? 'bg-blue-50 border-l-4 border-blue-500 font-semibold text-blue-700' : 'text-gray-600 hover:!bg-blue-200 hover:!text-blue-900 hover:font-medium hover:no-underline' }}">
