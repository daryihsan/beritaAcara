<div id="hasil" class="tab-pane fade">
    ...
    <div class="form-group">
        ...
        <textarea name="hasil_pemeriksaan" class="form-control" rows="5"
            required>{{ old('hasil_pemeriksaan', $isEdit ? $ba->hasil_pemeriksaan : '') }}</textarea>
    </div>
    <div class="form-group">
        <label>Nama Penanggung Jawab/Pemilik</label>
        <input type="text" name="yang_diperiksa" class="form-control" required
            value="{{ old('yang_diperiksa', $isEdit ? $ba->yang_diperiksa : '') }}"
            placeholder="Nama pimpinan/apoteker">
    </div>
